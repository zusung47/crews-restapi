...
public interface CrewListRepository extends JpaRepository<CrewList, CrewListId> {
    ...
    Page<CrewList> findByCrewAndApprovalStatus(Crew crew, Integer approvalStatus, Pageable paging);

    CrewList findByCrewAndUser(Crew crew, User user);

    List<CrewList> findByCrewAndApprovalStatus(Crew crew, int i);
}
